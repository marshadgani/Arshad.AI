...
| Field | Value |
|---|---|
| active_run_id | wf_ea9200d9-e33 (task `wy9hb1ud6`) |
| session | session_01S3fJzZzJykfHRA9jhMycuk |
| started | 2026-09-13 (this run) |
| FEAT-142 (OAuth invalid_state bug) | Launched 2026-09-13, fresh run — Arshad's mobile-Safari bug report on the login OAuth callback. In flight. |
...
| FEAT-142 | Bug: login OAuth callback fails with `invalid_state — "OAuth state does not match this browser session."` on `arshad-ai.onrender.com` (screenshot from mobile Safari, provider not confirmed from the screenshot alone — likely Google or GitHub login). | in_flight | — | — | Arshad's bug report (2026-09-13), screenshot showing the raw backend JSON error page rendered directly (not redirected through the frontend), with a `callback.json` download prompt — consistent with the callback hitting `backend/src/auth/routers.py::_handle_callback` and raising before any redirect. Root cause candidates identified from reading `backend/src/auth/routers.py` (lines 144-193, the SEC-002 CSRF-hardening design from FEAT-118): the login flow signs a nonce into the OAuth `state` param AND sets the same nonce as an `oauth_login_nonce` HttpOnly/SameSite=Lax cookie on the backend's own domain; the callback rejects with exactly this message when the cookie's nonce doesn't match the nonce embedded in `state` (line 179). Plausible triggers: (a) a double login attempt — tapping "Login"/"Connect" twice, or retrying after a slow network/back-button, calls `_start_login` twice, and the second cookie silently overwrites the first in the browser's cookie jar while an already-in-flight first OAuth round-trip (e.g. a stale Google account-picker tab) completes with the *first* nonce, so the state's nonce no longer matches the (now-second) cookie; (b) iOS Safari's Intelligent Tracking Prevention or a private-browsing/cookie-partitioning mode dropping the cookie between the redirect-to-provider and redirect-back-to-callback steps despite the same-domain top-level-navigation design intended to survive that (the docstring's own reasoning may not hold on iOS Safari specifically — needs verification, not assumed); (c) the user opened the login link in an in-app browser (e.g. from an email or notification) with a separate cookie jar from their main Safari session. Needs the debugger stage to reproduce/narrow which of these (or another cause) is actually happening, and the fix must preserve the SEC-002 CSRF protection this mechanism exists for — not simply relax or remove the cookie check. |
