...
**Launch plan for the next resume (once `wf_14899418-22d`'s current run settles):** combine FEAT-118, 119, 120 (cached, replay instantly if already completed) with FEAT-121, 122, 123, 124, 126, 127, 128, 130, 131, 132, 133, 134 (12 new features) in ONE `args.features` array. Hold FEAT-125 and FEAT-129 out of that batch — queue them for the batch after FEAT-119 shows `completed` in this file.

**Excluded from this batch (explicit decisions, do not re-add without a new instruction):** PayPal and ProfileMaster (Arshad said ignore), Coursera (pending Arshad's reconnect + verification that personal progress data is actually exposed, not just the public catalogue), Spotify as a standalone feature (assessed as novelty — only defensible as a 20-line side effect of something else, not its own FEAT), Uber Eats (API surface can't return personal order data), Lusha (no legitimate personal-dashboard use case), and Zomato's "burnout index" correlation (explicitly assessed as a toy, not worth building).
