...
| FEAT-143 (Obsidian skills registration + note-sync) | Launched 2026-09-14, run `wf_a51802fb-f0c`, running in background. |
...
| FEAT-143 | Obsidian integration, two parts: (1) register the already-synced `.claude/skills/obsidian-skills/` (defuddle, json-canvas, obsidian-bases, obsidian-cli, obsidian-markdown) via `scripts/register_skills.py` so they appear in the AI Ecosystem Skills tab; (2) add a note-sync integration — a new `data-pipeline`/`infrastructure` capability that writes Arshad.AI records (calendar, email, GitHub, decisions) out to an Obsidian vault as markdown notes via the Obsidian Local REST API or direct vault-file writes, so the vault mirrors portal data. | queued | — | — | **Relation to FEAT-141 (halted):** FEAT-141 already attempted a much larger "Obsidian second-brain ontology layer" and is halted on an unresolved Security Auditor escalation, with WIP preserved unmerged on `dev-team/feat-139-141-halted-wip`. FEAT-143 is deliberately scoped smaller (skill registration + a straightforward note-sync writer, not the full ontology/MOC graph) — BA and EA-pre should treat FEAT-141's halted design and its security finding as required reading before proposing an approach, and should explicitly confirm whether FEAT-143's narrower scope avoids the same security issue rather than re-triggering it. Arshad has not yet resolved FEAT-141's security escalation — do not silently fold FEAT-141's full scope into this feature. |
