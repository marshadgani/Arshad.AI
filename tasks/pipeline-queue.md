...
| FEAT-142 (OAuth invalid_state bug) | Settled 2026-09-13 — halted at Software Architect on the `backend/src/auth/**` denylist, but the System Engineer stage confirmed the root cause first (cross-origin cookie-scope bug, frontend-only 3-line fix identified). See Queue table row for details. Awaiting Arshad's sign-off to apply the fix. |
...
| FEAT-142 | Bug: login OAuth callback fails with `invalid_state — "OAuth state does not match this browser session."` on `arshad-ai.onrender.com`. | halted | — | — | **Halted: denylist violation at Software Architect** (`backend/src/auth/**`, `backend/src/auth/routers.py` are permanently denylisted from pipeline-driven changes — security-critical, needs human review, per `.claude/workflows/dev-team-pipeline.js`'s `DENY_PREFIXES`). Before halting, the pipeline's System Engineer stage produced a **CONFIRMED root cause via direct source inspection**, overriding the Solution Architect's earlier speculative design: this is a **structural cross-origin cookie-scope bug, not intermittent, not Safari-specific** — reproduces 100% of the time for every browser/provider/device. The login leg (`frontend/src/auth/AuthContext.tsx:79`) navigates to a *relative* path (`/api/v1/auth/${provider}/login`), so the top-level navigation targets the Vercel frontend origin, which proxies it server-side to the backend (`vercel.json` rewrite) — the browser's Set-Cookie for `oauth_login_nonce` lands on `arshad-ai-seven.vercel.app`. But the OAuth provider's callback redirect goes *directly* to `BACKEND_URL` (`arshad-ai.onrender.com`), bypassing Vercel entirely — two different registrable domains, so the cookie set on one is never sent on the other. Works in local dev only because `localhost:3000`/`:8000` share a cookie jar (cookies ignore port). **Minimal correct fix identified (not yet applied, needs Arshad's sign-off before touching auth-adjacent code): change `AuthContext.tsx`'s `loginWith` to navigate to the backend's absolute origin instead of a relative path** — 3 lines, frontend-only, doesn't touch any denylisted `backend/src/auth/*` file, preserves the SEC-002 CSRF cookie-check design exactly as originally intended. The System Engineer separately recommended deferring a proposed Redis "binder cookie" redesign (Solution Architect's W2) — confirmed unnecessary and riskier (breaking Redis value-format change, new failure window) since the cookie is *absent*, not *stale*. Full diagnosis in `/root/.claude/projects/-home-user-Arshad-AI/53ada982-c231-5135-8df0-2d319e25a31f/subagents/workflows/wf_ea9200d9-e33/journal.jsonl`. |
