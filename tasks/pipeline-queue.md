...
| FEAT-142 | Bug: login OAuth callback fails with `invalid_state — "OAuth state does not match this browser session."` on `arshad-ai.onrender.com`. **Root cause CONFIRMED, re-queued with a tightly scoped fix instruction** to avoid re-triggering the `backend/src/auth/**` denylist. | in_flight | — | — | Arshad approved the fix 2026-09-13 ("Ok"). Root cause (confirmed via direct source inspection by System Engineer in the first pipeline pass, run `wf_ea9200d9-e33`): a structural cross-origin cookie-scope bug — reproduces 100% of the time, every browser/provider/device, not intermittent, not Safari-specific. The login leg (`frontend/src/auth/AuthContext.tsx:79`) navigates to a *relative* path, so the top-level navigation targets the Vercel frontend origin (proxied server-side to the backend via `vercel.json`'s rewrite) — the `oauth_login_nonce` Set-Cookie lands on `arshad-ai-seven.vercel.app`. But the OAuth provider's callback redirect goes *directly* to `BACKEND_URL` (`arshad-ai.onrender.com`), bypassing Vercel — two different registrable domains, cookie set on one never sent on the other. Works in local dev only by accident (localhost:3000/:8000 share a cookie jar). **The fix is exactly this and nothing else: change `AuthContext.tsx`'s `loginWith` to navigate to the backend's absolute origin (`import.meta.env.VITE_API_BASE_URL` or equivalent) instead of a relative `/api/...` path, for both the Google and GitHub login buttons.** This is a frontend-only change, ~3 lines, touches no `backend/src/auth/*` file, and preserves the existing SEC-002 CSRF cookie-check design exactly as originally intended — do not redesign the cookie/state mechanism, do not touch `backend/src/auth/routers.py` or any file under `backend/src/auth/`, do not add a "binder cookie" or any new backend auth module (the first pipeline pass's Solution Architect proposed exactly that and it was both unnecessary — System Engineer confirmed the cookie is *absent*, not *stale* — and is what tripped the denylist halt last time). If the debugger/architecture stages want to touch anything under `backend/src/auth/`, treat that as a signal the diagnosis needs re-verification, not as license to proceed — stop and flag it instead. Full first-pass diagnosis: `/root/.claude/projects/-home-user-Arshad-AI/53ada982-c231-5135-8df0-2d319e25a31f/subagents/workflows/wf_ea9200d9-e33/journal.jsonl`. |
