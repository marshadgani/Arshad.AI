...
| FEAT-144 | Follow-up from FEAT-143's Merge-to-Main gate report (all WARN-level, none blocking, none part of the original council's approved scope for those specific selectors): (1) `Integrations.module.css` still has undefined `var(--border-hover, #444c56)`, `var(--mono, ...)`, and unguarded `var(--bg)`/`var(--bg-hover)` (no fallback — resolves to transparent) on ~7 selectors the council's token-repoint pass missed on this one file; (2) `AgentCard.module.css`'s `.modelOpus`/`.modelSonnet` badges now render byte-identical (lost the Opus vs Sonnet visual distinction) — needs a design decision, not just a mechanical fix; (3) `tokens.css`'s `--text-muted` fix compressed its contrast gap from `--text-secondary` to ~1.17:1 — consider nudging `--text-secondary` lighter; (4) `FundFlowMap.module.css`'s new `.canvasWrap::after` scroll-fade renders at 0px height (non-functional, harmless) due to `height: 100%` on a `float` element inside an auto-height parent; (5) add a `tokens.test.ts` static guard (sibling to `orphans.test.ts`) asserting every `var(--x)` in `src/**/*.module.css` has a matching definition in `tokens.css`, to catch this exact bug class at introduction next time; (6) `Dashboard.module.css`'s `.checkbox::before` hit-area is dead code (no `.tsx` references `styles.checkbox`) — wire it up or remove it; (7) standardize `prefers-reduced-motion` guard direction (prefer `no-preference`-gated) across AppleHealthCard/Integrations/Obsidian, currently mixed; (8) add the 4 WHY-comments doc-writer identified (`--text-muted` WCAG floor rationale, `fadeUp`'s missing reduced-motion guard or documented exemption, Obsidian's `position: sticky` mechanic, the `vh`/`dvh` progressive-enhancement pattern). | queued | — | — | Full detail in `tasks/last-gate-report.md`'s Action Items section. All items are style-only / test-only, consistent with FEAT-143's constraint (no feature/structure changes) except item 2, which needs Arshad's input on how Opus/Sonnet/Haiku should be visually distinguished before it can be mechanically applied. |
