...
| FEAT-145 | Follow-up from FEAT-144's Merge-to-Main gate report — 2 items, both test-only: (1) widen `frontend/src/styles/tokens.test.ts`'s scan scope to also cover `globals.css` and inline `var(--x)` usage in `.tsx` style props (currently only scans `*.module.css` — both are clean today but unguarded, a latent gap several gate agents independently flagged); (2) add a self-verifying fixture test to `tokens.test.ts` proving its regex logic can actually catch a violation (a synthetic undefined `var()` reference), so the guard can't silently pass vacuously if its own parsing logic ever breaks. | queued | — | — | Full detail in `tasks/last-gate-report.md`'s Action Items section (FEAT-144's report). Test-only — no CSS/behavior change, so no council/design decision needed; safe to apply directly like FEAT-144 was. |
