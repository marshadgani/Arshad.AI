...
| FEAT-141 | Turn Obsidian into a "second brain" that mirrors all data flowing into the Arshad.AI portal — every ingested/derived record (Gmail threads, GitHub activity, Calendar events, finance holdings, weather, tasks/decisions/notifications, etc.) should also land as a note in the connected Obsidian vault, not just the current one-way vault→Postgres sync. | queued | — | — | Arshad's explicit request (2026-09-13), right after connecting `OBSIDIAN_GITHUB_REPO=marshadgani/Obsidian-Vault`. Current `backend/src/services/obsidian_client.py` + `api/v1/obsidian.py` only pull FROM the vault into `ingested_obsidian_notes` and let chat tools write individual notes — there is no automatic push path from the ingestion pipeline (email/github/calendar/finance/weather) INTO the vault. This needs a design decision on note structure/foldering per data domain, idempotent upsert-by-path (avoid duplicate vault commits on every ingestion run), and rate-limit awareness (GitHub Contents API, one commit per file write) — squarely a Solution-Architect/System-Engineer decision, not a direct code change. Depends on FEAT-136/138/139 (the ingestion sources it would mirror) being live first for the highest-value domains, but can be scoped to start with whatever is already ingested (Obsidian's own existing sync) and grow from there — does not need to block on every one of them completing. |
