...
| Field | Value |
|---|---|
| active_run_id | wf_47ee05d4-40f (task wt00038xp) — FEAT-143 only |
| session | session_01VPKZ737Jp9yAXDMTRguAa2 |
| started | 2026-09-14 (this run) |
...
| FEAT-143 | Add username/password login as an additional auth method alongside the existing Google/GitHub OAuth — one combined login screen offering both: user can either enter username+password, or click "Continue with Google"/"Continue with GitHub". Do NOT remove/replace OAuth. Backend needs a password-based auth path (hashed password storage, login/register endpoints) issuing the same JWT session the OAuth path already issues, sharing the existing user model/session mechanism. Arshad's own spoken requirement (session 2026-09-14): keep OAuth code intact and working, add password auth as a parallel option, single unified login UI presenting both choices. | in_flight | — | — | Fresh `dev-team-pipeline` Workflow run started 2026-09-14, run `wf_47ee05d4-40f` (task `wt00038xp`). Active run for this session — see "Active run" note below. |
