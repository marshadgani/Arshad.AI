...
| resume batch | Launched 2026-09-07 (task `w9kn2psyx`, **currently running**): FEAT-119 (continuing past its convergence-test success) + FEAT-120 (denylist exception applied, retrying its own redesign pass) using the fixed redesign-loop prompt and the self-correcting Process Organiser retry. FEAT-121 deliberately NOT included — held for a human decision on scope/approach first. |
...
| FEAT-119 | Shopify integration — live store confirmed to have real orders. | in_flight | dev-team/feat-119-* (TBD) | — | Was Architecture Critic-blocked (broken `connect()` signature, unpersisted `shop_domain`, timezone/throttling bugs) even after 2 redesign rounds. **Convergence test (`wm5xz1tv5`) succeeded** — got past Architecture Critic with the fixed redesign prompt. Now continuing through the rest of the pipeline (post-ArchCritic stages) in task `w9kn2psyx`, currently running. |
| FEAT-120 | Whoop + Apple Health for Health & Fitness. | in_flight | — | — | Denylist violation: an agent wrote `backend/src/middleware/rate_limit.py` directly to disk via its own Write tool, bypassing the script's JSON-based check. Content reviewed directly — safe, a legitimate Whoop-rate-limiter extraction now shared with the Apple Health ingest route. **Kept per explicit decision** (commit `edccc1f` added `DENY_EXCEPTIONS` for this exact path); the denylist enforcement gap itself is a known unresolved issue (script has no filesystem access to verify actual disk state — see note below). Relaunched in task `w9kn2psyx`, currently running, using the fixed redesign-loop prompt. |
