...
| FEAT-141 | Turn Obsidian into a "second brain" that mirrors all data flowing into the Arshad.AI portal, organised as an **ontology layer** — not flat per-record notes, but linked entities and relationships (people, projects, threads, events, holdings, etc.) with tags and Maps-of-Content (MOCs) tying domains together, so the vault reads as a navigable knowledge graph rather than a data dump. | queued | — | — | Arshad's explicit request (2026-09-13), right after connecting `OBSIDIAN_GITHUB_REPO=marshadgani/Obsidian-Vault`, then refined further (2026-09-13): the sync must visibly produce an ontology layer, not just isolated notes. Current `backend/src/services/obsidian_client.py` + `api/v1/obsidian.py` only pull FROM the vault into `ingested_obsidian_notes` and let chat tools write individual notes — there is no automatic push path from the ingestion pipeline (email/github/calendar/finance/weather) INTO the vault, and no entity/relationship model at all yet. Design must define: (1) an entity schema per domain (e.g. Person, Thread, Event, Holding, Repo) with stable note IDs so the same real-world entity always resolves to the same note across syncs, (2) `[[wikilink]]`-based relationships between entities (e.g. a Gmail thread note links to the Person notes of its participants), (3) per-domain MOC index notes that aggregate/cross-link their entities, (4) YAML frontmatter carrying structured properties (type, tags, source, last_synced) so Obsidian's own graph view and Dataview-style queries work against it, (5) idempotent upsert-by-path to avoid duplicate vault commits on every ingestion run, (6) GitHub Contents API rate-limit awareness. Squarely a Solution-Architect/System-Engineer design decision, not a direct code change. Depends on FEAT-136/138/139 (the ingestion sources it would mirror) being live first for the highest-value domains, but can be scoped to start with whatever is already ingested and grow from there — does not need to block on every one of them completing. |
