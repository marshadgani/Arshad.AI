...
| Field | Value |
|---|---|
| active_run_id | none — settled 2026-09-14 (halted at denylist, 3rd attempt) |
...
| FEAT-143 | Add username/password login as an additional auth method alongside the existing Google/GitHub OAuth — one combined login screen offering both: user can either enter username+password, or click "Continue with Google"/"Continue with GitHub". Do NOT remove/replace OAuth. **Amended 2026-09-14 after Architecture Critic halt** (see notes) — explicit fixes required: (1) dummy_verify must run ONLY on branches where no real bcrypt executed (user not found, or password_hash IS NULL) — invariant: exactly one bcrypt op per request on every branch, so wrong-password and unknown-email/OAuth-only take equal time; (2) rate limiting must be per-IP (~20/min) as the primary guard with a higher global bucket as backstop only, and must fail closed (not open) on a Redis outage, with OAuth documented as the degradation path; (3) the timing-equalisation test must be a deterministic spy asserting bcrypt-performing call_count == 1 on each failure branch — no wall-clock/sleep-based assertions; (4) registration decision: Arshad approved dropping the HTTP registration endpoint in favor of `scripts/set_password.py` run directly against Render (single-user app, no benefit to an open registration endpoint) — this is the final approved design, write it up as an explicit documented amendment plus a runbook for running the script against Render (note: DATABASE_URL's pooler is fine for this single UPDATE, unlike Alembic migrations per CLAUDE.md §23). | halted | `dev-team/feat-143-password-auth-halted-denylist` | — | **3rd attempt, 2026-09-14 — design converged, code is real and solid, but blocked by the pipeline's own denylist, not a quality issue.** All 4 prior review findings resolved: per-email fail-closed lockout, per-IP rate limiting rejected with real cited evidence (`frontend/vercel.json`'s external rewrite + `oauthLoginUrl.ts` prove Render is directly reachable, so XFF can't be trusted — see EV-1 in the journal), single bcrypt call site with `dummy_verify` on the correct branches only, centralized `normalize_email()`, password stripped from 422 bodies. **Blocked because `backend/src/auth/{password,lockout,service,routers}.py` are on the pipeline's permanent `DENY_PREFIXES` list** (security-critical code requires Arshad's own review before any exception — see the FEAT-120 precedent comment in `.claude/workflows/dev-team-pipeline.js`). All 16 generated files (the 5 auth/* ones plus 11 non-denylisted ones — models, migration, `set_password.py`, tests, full frontend Login screen) preserved on `dev-team/feat-143-password-auth-halted-denylist`, **not merged, not built on further**. See `tasks/alternate-features.md` FEAT-143 (3rd attempt) row. **Needs Arshad's personal review + explicit approval of the 5 auth/* files before this can proceed** (denylist exceptions are never self-granted). |
