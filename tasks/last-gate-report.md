...
- [x] Write unit tests for `_compress_history` (4 paths: under budget, trim, single-user break, empty) ← fixed in commit `e914951`
- [x] Write unit tests for `refresh_google_token` (success, 400/401, missing row, rotation, None expiry) ← fixed in commit `e914951`
...
- [x] Fix denylist: `backend/src/auth/*` → `backend/src/auth/**` ← fixed in commit `e914951`
- [x] Fix denylist: add `**/.env*` alongside `*.env*` ← fixed in commit `e914951`
- [x] Add `security_halt` check before Step 10 in orchestrator.md ← fixed in commit `e914951`
- [x] Add EA post-build `decision: rejected` halt before Step 10 ← fixed in commit `e914951`
...
- [x] Fix `gate.md` Step 0: `main` → `claude/ai-personal-assistant-main` ← fixed in commit `e914951`
...
*Gate verdict: BLOCKED — 1 Critical remaining (broad coverage gap on 108 Python files)*
