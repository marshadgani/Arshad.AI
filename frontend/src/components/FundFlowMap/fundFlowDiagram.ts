import { type TrustedStaticSvg, trustedStaticSvg } from '../../utils/trustedStaticSvg';

/* Static, hand-maintained reference material — not a live data view (see the
   "Manual" badge and caption in FundFlowMap). It stays raw SVG rather than
   JSX because it hardcodes pixel coordinates across ~200 elements, where
   hand-converting attributes to camelCase would be error-prone.

   If this is ever wired to real data, render it as JSX/SVG elements instead:
   never interpolate into this string, and never drop `trustedStaticSvg` or
   the `TrustedStaticSvg` type — together they are what stop this
   dangerouslySetInnerHTML sink from accepting user input. */
export const FUND_FLOW_DIAGRAM_SVG: TrustedStaticSvg = trustedStaticSvg`<svg xmlns="http://www.w3.org/2000/svg" width="1820" height="1580" viewBox="0 0 1820 1580">
<defs>
