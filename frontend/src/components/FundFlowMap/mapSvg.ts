/* The hand-drawn fund-flow diagram, as raw SVG markup.

   It depicts real personal financial topology - employer, vendors, banks,
   family and recurring payments - but nothing reads an account, transaction
   or balance to draw it: it is hand-maintained and NOT synced to any
   integration or API. Provenance lives in ./mapMeta (bump it in the same
   commit whenever this drawing is redrawn); the user-facing "Static"
   labelling lives in ./FundFlowMap.

   Raw SVG rather than JSX because the drawing hardcodes ~200 pixel-positioned
   elements and the Space Mono + Syne fonts loaded in index.html - converting
   every attribute to JSX camelCase would be error-prone for no gain while the
   content is static.

   This literal must stay free of interpolation: the moment a value is
   substituted into it, the dangerouslySetInnerHTML that renders it becomes an
   XSS sink. Binding real fund-flow data later means converting the drawing to
   JSX, never interpolating here. */
export const MAP_SVG = `<svg xmlns="http://www.w3.org/2000/svg" width="1820" height="1580" viewBox="0 0 1820 1580">
<defs>
