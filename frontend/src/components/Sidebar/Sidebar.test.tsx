...
describe('Sidebar', () => {
  ...
  it('renders no dead links and keeps the Integrations link functional', async () => {
    setViewport(1280);
    renderSidebar(false, false);
    await waitFor(() => expect(screen.getByText('Home')).toBeInTheDocument());
    ...
    const accountHrefs: Array<[RegExp, string]> = [
      [/Integrations/i, '/integrations'],
      [/Activity log/i, '/activity-log'],
      [/Settings/i, '/settings'],
    ];
    accountHrefs.forEach(([name, href]) => {
      expect(screen.getByRole('link', { name })).toHaveAttribute('href', href);
    });
  });

  it('drops nav rows whose destination is not an in-app path', async () => {
    setViewport(1280);
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
      json: async () => ({
        data: [
          { to: '/', label: 'Home', icon: '\u2302' },
          { to: '//evil.example', label: 'Protocol relative', icon: 'x' },
          { to: 'https://evil.example', label: 'Absolute', icon: 'x' },
          { to: 'javascript:alert(1)', label: 'Script URL', icon: 'x' },
        ],
      }),
    }) as unknown as typeof fetch;
    ...
    // A nav row is DB data, so an off-site or script destination must never
    // reach the DOM as a link the user can click.
    ...
    expect(screen.queryByText('Script URL')).not.toBeInTheDocument();
    screen.getAllByRole('link').forEach((a) => {
