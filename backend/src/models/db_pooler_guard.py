...
def reject_transaction_mode_pooler(db_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            "Could not parse a port from the database URL — if the password "
            "contains '@', ':', or '/', percent-encode it."
        ) from None

    hostname = parsed.hostname or ""
    if "pooler.supabase.com" in hostname and port == TRANSACTION_MODE_PORT:
        raise RuntimeError(
            "DATABASE_URL points at Supabase's Transaction mode pooler "
            f"({hostname}:{TRANSACTION_MODE_PORT}), which is incompatible with asyncpg.\n\n"
            "Fix on Render:\n"
            "  1. Go to Supabase dashboard → Project Settings → Database → Connection string\n"
            "  2. Select the 'Session mode' pooler connection string (port 5432)\n"
