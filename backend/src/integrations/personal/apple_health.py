...
class AppleHealthIntegration(IntegrationProvider):
    ...
    icon = "apple-health"
    # FEAT-145: the credential is a locally-minted ingest bearer token, not
    # a third-party grant, so there is nothing upstream to revoke. The base
    # disconnect() revokes the token locally (scrub_credentials sets
    # IntegrationIngestToken.revoked_at), which is what the hand-written
    # override that used to live here did.
