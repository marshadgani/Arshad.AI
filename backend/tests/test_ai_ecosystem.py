...
# ---------------------------------------------------------------------------
# GET /api/v1/ai-ecosystem/metrics
# ---------------------------------------------------------------------------
#
# Regression coverage for FEAT-161: the endpoint's cast(AgentUsageLog.success,
# Integer) inside func.avg() is a Python expression built at request time
# (not query-compile time), so a missing `Integer` import raises a NameError
# the instant this route runs — a bug that reached a commit on this branch
# because nothing here ever actually called get_metrics(). A mocked
# db.execute() still exercises that code path: the select(...) statement is
# built in Python before it's ever handed to execute(), so this catches the
# exact class of bug (undefined name in the query-building code) even though
# it never touches a real Postgres.


def _fake_row(agent_name, usage_count, total_tokens, avg_tokens, success_rate):
...
def metrics_client(monkeypatch):
    """TestClient with auth + DB stubbed out. Yields a function that takes
    the fake rows db.execute(...).all() should return and hands back a
    ready-to-use TestClient; overrides are cleared on teardown either way."""
    import uuid
...
class TestGetMetricsEndpoint:
    def test_metrics_endpoint_returns_200_and_does_not_nameerror(self, metrics_client):
        # This is the exact regression: before the Integer import fix, this
        # call raised NameError instead of returning a response at all.
        client = metrics_client([_fake_row("code-reviewer", 3, 900, 300, 0.5)])
        resp = client.get("/api/v1/ai-ecosystem/metrics?period=1d")
    ...
    def test_metrics_endpoint_reports_a_genuine_zero_success_rate_as_zero(self, metrics_client):
        ...
        client = metrics_client([_fake_row("bug-fixer", 4, 400, 100, 0.0)])
        resp = client.get("/api/v1/ai-ecosystem/metrics?period=1d")
        assert resp.status_code == 200
        assert resp.json()["data"]["agents"][0]["success_rate"] == 0.0

    def test_metrics_endpoint_handles_no_usage_rows(self, metrics_client):
